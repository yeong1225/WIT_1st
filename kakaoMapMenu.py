...
def main():
    ...
    # CSV 파일 읽기
    data = pd.read_csv("전남대상대맛집.csv")

    # 특정 열의 데이터 추출
    column_data = data['placetitle']

    # 2번 행부터의 데이터
    extracted_data = column_data[2:]

    # 랜덤으로 데이터 추출
    random_restaurant_hu = np.random.choice(extracted_data)

    search(random_restaurant_hu)

    driver.quit()
    print(random_restaurant_hu)
    print("finish")
